...
UV ?= uv
# 端口(dev-graph 可覆盖:PORT=8090 make dev-graph)。
PORT ?= 8080
...
help:
	...
	@echo "make dev-graph       # start agent-gateway WITH real KB graph (lightrag extra; PORT=8090 to override)"
...
dev: sync
	$(UV) run python -m uvicorn agent_gateway.app:app --host 127.0.0.1 --port 8080 --ws none

# 起带真实知识图谱的网关:装 lightrag 可选 extra 并用 --no-sync 起(防 uv sync 卸掉 extra)。
# 需真实 LLM(config/llm.yml 非 dev_stub)+ config/app.yml FP_KB_GRAPH_ENGINE=lightrag。PORT 可覆盖。
dev-graph:
	$(UV) sync --all-packages --extra lightrag
	$(UV) run --no-sync python -m uvicorn agent_gateway.app:app --host 127.0.0.1 --port $(PORT) --ws none
