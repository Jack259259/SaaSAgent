...
——**上传 → 入库(ingest)→ 下载 / 删除**。镜像既有「代码仓管理」(`docs/integration/code-repos.md`)形态。
...
| DELETE | `/admin/kb/{kb}/docs/{name}` | 删除文档(物理文件 + 检索索引;入库中→`409`;**不触发图谱重建**,靠下次 ingest 同步) |
...
- 写操作(upload / ingest / download / delete)全量审计(`emit_audit`,tool=`kb_admin:<action>`;不记文件名原文)。
...
- **路径穿越防护**:文件名取 `Path(name).name`(去目录成分)+ 拒 `..`/分隔符/空 + 扩展名白名单
  ...
  下载与**删除**同走 `doc_path` 这一套防护。
- **删除与 ingest 互斥**:`delete` 全程持 `_jobs_lock`(与 `start_ingest` 注册互斥),入库进行中 →
  `409 INGEST_RUNNING`;**先清检索索引(chunks + file_hash 持久化)再物理删文件**——增量 ingest 不清理
  缺失文件的旧 chunk,反序失败会留下检索仍可命中的孤儿 chunk,正序失败仅 `indexed` 翻 False 可重试自愈。
  删除**不重建知识图谱**(LightRAG 仅支持全量重建):图谱靠下次「重新入库」同步,前端确认弹窗已提示。
