...
**开启真实知识图谱**(默认 mock,不建图)需四者同时满足:`config/app.yml` 设 `FP_KB_GRAPH_ENGINE: lightrag` + `FP_KB_GRAPH≠0`(默认开)+ `pip install lightrag-hku` + 配置真实 LLM(开发桩 `dev_stub`/`MockProvider` 会跳过建图)。详见 `docs/integration/backend-gaps.md` §I。
