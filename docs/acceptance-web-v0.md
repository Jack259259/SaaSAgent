...
| W4 Skill 管理 | ✅ | 角色门控入口 + 列表/详情(消毒)/编辑/删除/新建弹窗(schema 校验失败)/上传 zip(逐条);非授权入口隐藏;对 mock(后端 /skills 待补) |

- **selftest:62 passed / 0 failed**(令牌 / SSE 解析 / currentRun 投影 / 折叠分档 / Markdown 消毒 / 流式 / 文件校验 / Repository CRUD / 附件 payload / Skill 角色门控·校验·API)。
